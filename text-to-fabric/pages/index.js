...
export default function Home() {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    // Static export: API routes don't exist. Always use client-side when key is available.
    const apiKey = typeof window !== 'undefined' && process.env.NEXT_PUBLIC_OPENAI_API_KEY;

    try {
      // Use client-side OpenAI call (required for static/GitHub Pages deployment)
      if (apiKey) {
        ...
        if (key && analyzeTextClientSide) {
          try {
            const data = await analyzeTextClientSide(input, key);
            setResult(data);
            setLoading(false);
            return;
          } catch (clientError) {
            console.error("Client-side API Error:", clientError);
            alert(clientError.message || "Failed to analyze");
            setLoading(false);
            return;
          }
        }
        alert("API key not configured. Add NEXT_PUBLIC_OPENAI_API_KEY to .env.local and rebuild.");
        setLoading(false);
        return;
      }

      // Fallback: try server-side API route (only works when not using static export)
